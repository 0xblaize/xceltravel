...
const NavLink = ({ label, onClick, active }: any) => (
  <button
    onClick={onClick}
    className={`transition-all text-[10px] font-black uppercase tracking-[0.2em] ${active ? "text-[#d9a321] border-b-2 border-[#d9a321] pb-1" : "text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white"}`}
  >
    {label}
  </button>
);
...
const ActionButton = ({ icon: Icon, label, onClick }: any) => (
  ...
  >
    ...
    <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-gray-400 group-hover:text-[#d9a321] transition-colors">
      {label}
    </span>
  </div>
);
  ...
  <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 px-3 rounded-2xl transition-all cursor-pointer">
    <div className="flex items-center gap-4">
      ...
      <div className="text-left">
        <p className="font-black text-[13px] uppercase italic text-slate-900 dark:text-white leading-none">
          {name}
        </p>
        <p className="text-[9px] text-slate-400 dark:text-gray-500 font-bold uppercase mt-1 tracking-wider">
          {chain}
        </p>
      </div>
    </div>
    <div className="text-right flex flex-col items-end">
      {isLoading ? (
        ...
        <>
          <p className="font-black text-sm text-slate-900 dark:text-white italic">
            {balance} {symbol}
          </p>
          <p className="text-[10px] text-slate-400 dark:text-gray-400 font-bold mt-1 tracking-tighter">
            {fiatValue}
          </p>
        </>
      )}
...
const HistoryRow = ({ type, title, amount, date, status }: any) => {
  ...
  return (
    <div className="flex items-center justify-between py-4 border-b border-slate-100 dark:border-white/5 last:border-0 hover:bg-slate-50 dark:hover:bg-white/5 px-3 rounded-2xl transition-all">
      <div className="flex items-center gap-4">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center ${isSend ? "bg-red-500/10 text-red-500" : "bg-green-500/10 text-green-500"}`}
        >
          {isSend ? <ArrowUpCircle size={20} /> : <ArrowDownCircle size={20} />}
        </div>
        <div className="text-left">
          <p className="font-black text-[13px] uppercase italic text-slate-900 dark:text-white leading-none truncate w-32 md:w-48">
            {title}
          </p>
          <p className="text-[9px] text-slate-400 dark:text-gray-500 font-bold uppercase mt-1 tracking-wider">
            {date}
      ...
      <div className="text-right">
        <p
          className={`font-black text-sm italic ${isSend ? "text-slate-900 dark:text-white" : "text-green-500"}`}
        >
          {amount}
        </p>
        <p className="text-[9px] text-slate-400 dark:text-gray-400 font-bold mt-1 tracking-tighter">
          {status}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-[#050a12] w-full h-[90vh] md:h-auto max-w-md rounded-t-[40px] md:rounded-[40px] p-8 border border-slate-200 dark:border-white/10 shadow-2xl relative animate-in slide-in-from-bottom-10 md:slide-in-from-bottom-0 md:zoom-in-95 flex flex-col">
        ...
        <h3 className="text-2xl font-black italic uppercase text-[#d9a321] mb-8">
          Send Crypto
        </h3>
        <div className="space-y-6 flex-1">
          <div className="flex items-center justify-between bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-5 rounded-2xl cursor-pointer hover:border-[#d9a321] transition-all">
            <div className="flex items-center gap-3">
              ...
              />
              <div className="text-left">
                <span className="font-black text-sm italic uppercase text-slate-900 dark:text-white block">
                  Ethereum (ETH)
                </span>
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-5 rounded-2xl text-sm font-mono outline-none focus:border-[#d9a321] text-slate-900 dark:text-white"
          />
          <div className="relative">
            <input
              placeholder="0.00"
              type="number"
              className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-2xl text-4xl font-black italic outline-none focus:border-[#d9a321] text-slate-900 dark:text-white"
            />
            <span className="absolute right-6 top-1/2 -translate-y-1/2 font-black text-slate-400 text-sm">
              MAX
            </span>
          </div>
        </div>
        <button className="w-full bg-[#d9a321] text-black py-5 rounded-2xl font-black uppercase text-sm tracking-widest shadow-xl hover:bg-[#b5871b] transition-colors mt-8">
          Preview Send
        </button>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-[#050a12] w-full h-[90vh] md:h-auto max-w-md rounded-t-[40px] md:rounded-[40px] p-8 border border-slate-200 dark:border-white/10 shadow-2xl relative animate-in slide-in-from-bottom-10 md:slide-in-from-bottom-0 md:zoom-in-95 flex flex-col">
        ...
        <h3 className="text-2xl font-black italic uppercase text-[#d9a321] mb-6">
          Receive Assets
        </h3>
        <div className="flex bg-slate-100 dark:bg-white/5 rounded-xl p-1 mb-8">
          <button
            onClick={() => setNetwork("BASE")}
            className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${network === "BASE" ? "bg-[#d9a321] text-black shadow" : "text-slate-400"}`}
          >
            Base
          </button>
          <button
            onClick={() => setNetwork("ETH")}
            className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${network === "ETH" ? "bg-[#d9a321] text-black shadow" : "text-slate-400"}`}
          >
            Ethereum
          </button>
          <button
            onClick={() => setNetwork("SOL")}
            className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${network === "SOL" ? "bg-[#d9a321] text-black shadow" : "text-slate-400"}`}
          >
            Solana
          </button>
        </div>
        <div className="flex flex-col items-center space-y-8 flex-1 justify-center">
          <div className="p-4 bg-white rounded-[2rem] shadow-sm border border-slate-100 relative">
            <img
              ...
              className={`w-48 h-48 ${!rawAddress && "opacity-10 blur-sm"}`}
              alt="QR"
            />
            {!rawAddress && (
              <p className="text-xs text-red-500 font-bold text-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                Wallet Generating...
              </p>
            )}
          </div>
          <div className="w-full text-center">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d9a321] mb-3">
              Your {network} Address
            </p>
            <div
              onClick={handleCopy}
              className={`bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-5 rounded-2xl flex justify-between items-center transition-all ${rawAddress ? "cursor-pointer group hover:border-[#d9a321]" : "opacity-50 cursor-not-allowed"}`}
            >
              <span className="text-xs font-mono text-slate-500 dark:text-gray-400 truncate w-64">
                {displayAddress}
              </span>
              {copied ? (
                ...
                <Copy
                  size={20}
                  className="text-slate-400 group-hover:text-[#d9a321]"
                />
              )}
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-[#050a12] w-full h-[90vh] md:h-auto max-w-md rounded-t-[40px] md:rounded-[40px] p-8 border border-slate-200 dark:border-white/10 shadow-2xl relative animate-in slide-in-from-bottom-10 md:slide-in-from-bottom-0 md:zoom-in-95 flex flex-col">
          ...
          <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-3xl">
            ...
            <div className="flex justify-between items-center">
              <input
                placeholder="0"
                type="number"
                className="bg-transparent text-4xl font-black italic outline-none text-slate-900 dark:text-white w-1/2"
              />
          ...
          <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 rounded-3xl mt-2">
            ...
            <div className="flex justify-between items-center">
              <input
                placeholder="0"
                disabled
                className="bg-transparent text-4xl font-black italic outline-none text-slate-500 w-1/2"
              />
              <div className="flex items-center gap-2 bg-white dark:bg-[#050a12] px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm cursor-pointer">
                <img
        ...
        <button className="w-full bg-[#d9a321] text-black py-5 rounded-2xl font-black uppercase text-sm tracking-widest shadow-xl hover:bg-[#b5871b] transition-colors mt-6">
          Review Swap
        </button>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white dark:bg-[#050a12] w-full h-[90vh] md:h-[600px] max-w-md rounded-t-[40px] md:rounded-[40px] p-8 border border-slate-200 dark:border-white/10 shadow-2xl relative animate-in slide-in-from-bottom-10 md:slide-in-from-bottom-0 flex flex-col">
        ...
        <h3 className="text-xl font-black italic uppercase text-[#d9a321] mb-6">
          Manage Tokens
        </h3>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
          {tokenData.map((token: any) => (
            <div
              key={token.id}
              className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-transparent hover:border-slate-200 dark:hover:border-white/10 transition-all"
            >
              <div className="flex items-center gap-3">
                <img
                  src={token.icon}
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full relative transition-colors duration-300 ${token.active ? "bg-[#d9a321]" : "bg-slate-300 dark:bg-slate-700"}`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all duration-300 shadow-sm ${token.active ? "left-[26px]" : "left-0.5"}`}
                ></div>
              </button>
            </div>
          ))}
          {search.startsWith("0x") && (
            <div className="mt-6 p-6 border border-dashed border-[#d9a321]/50 rounded-3xl text-center bg-[#d9a321]/5 cursor-pointer hover:bg-[#d9a321]/10 transition-all">
              ...
              <p className="text-xs font-black uppercase italic text-[#d9a321]">
                Import Custom Token
              </p>
              <p className="text-[9px] text-slate-500 mt-1 font-mono">
                {search.substring(0, 16)}...
              </p>
            </div>
          )}
        </div>
...
export default function WalletPage() {
  ...
  const [activeTab, setActiveTab] = useState<"tokens" | "nfts" | "activity">(
    "tokens",
  );
      ...
      try {
        ...
        if (isMounted) {
          setTokenData((prev) =>
            prev.map((t) =>
              t.symbol === "SOL"
                ? { ...t, balance: liveSolBalance, fiat: "$0.00" }
                : t,
            ),
          );
  ...
  if (!ready || (authenticated && !wallets.length)) {
    return (
      <div className="min-h-screen bg-[#050a12] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-[#d9a321] text-[10px] font-black uppercase tracking-[0.3em] italic animate-pulse">
          ...
          <div className="flex items-center justify-between mb-6 border-b border-slate-200 dark:border-white/10 px-2 pb-4">
            <div className="flex gap-8">
              <button
                ...
                className={`text-[11px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === "tokens" ? "text-[#d9a321]" : "text-slate-400 hover:text-slate-300"}`}
              >
                Tokens
              </button>
              <button
                onClick={() => setActiveTab("nfts")}
                className={`text-[11px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === "nfts" ? "text-[#d9a321]" : "text-slate-400 hover:text-slate-300"}`}
              >
                NFTs
              </button>
              <button
                onClick={() => setActiveTab("activity")}
                className={`text-[11px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === "activity" ? "text-[#d9a321]" : "text-slate-400 hover:text-slate-300"}`}
              >
                Activity
              </button>
            </div>
            {activeTab === "tokens" && (
              <button
                ...
                className="text-slate-400 hover:text-[#d9a321] transition-colors flex items-center gap-1"
              >
                <Settings2 size={16} />
              </button>
            )}
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            {activeTab === "nfts" && (
              <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in">
                <div className="w-16 h-16 rounded-full bg-[#d9a321]/10 flex items-center justify-center text-[#d9a321] mb-4">
                  <ImageIcon size={24} />
                </div>
                <h4 className="text-sm font-black italic uppercase tracking-widest text-slate-900 dark:text-white">
                  No NFTs Found
                </h4>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-2">
                  Collect digital assets to view them here
                </p>
              </div>
            )}
